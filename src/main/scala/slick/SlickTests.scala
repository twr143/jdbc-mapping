package slick
/**
  * Created by Ilya Volynin on 03.12.2018 at 9:24.
  */
...
trait Queries extends Schema {
  ...
  def insertWithGeneratedId()(implicit logger: Logger): Future[Unit] = {
    val insertQuery = (cities returning cities.map(_.id)) += City(CityId(0), "New York", 19795791, 141300, None)
    db.run(insertQuery).map { r =>
      logger.warn(s"Inserted, generated id: $r")
      logger.warn(s"\n")
    }
  }

  def selectAll()(implicit logger: Logger): Future[Unit] = {
    val query = cities.result
    runAndLogResults("All cities", query, query)
  }

  def selectNamesOfBig()(implicit logger: Logger): Future[Unit] = {
  ...
  // many-to-one
  def selectMetroSystemsWithCityNames()(implicit logger: Logger): Future[Unit] = {
  ...
  def selectMetroLinesSortedByStations()(implicit logger: Logger): Future[Unit] = {
  ...
  def selectMetroSystemsWithMostLines()(implicit logger: Logger): Future[Unit] = {
  ...
  // one-to-many
  def selectCitiesWithSystemsAndLines()(implicit logger: Logger): Future[Unit] = {
  ...
  def selectLinesConstrainedDynamically()(implicit logger: Logger): Future[Unit] = {
  ...
  def plainSql()(implicit logger: Logger): Future[Unit] = {
    case class MetroSystemWithCity(metroSystemName: String, cityName: String, dailyRidership: Int)
  ...
  */
  def transactions()(implicit logger: Logger): Future[Unit] = {
  ...
  /**
    ...
    */
  private def runAndLogResults[R](label: String, sqlQuery: SqlAction[Seq[R], NoStream, _], query: DBIOAction[Seq[R], NoStream, _])(implicit logger: Logger): Future[Unit] = {
    db.run(query).map { r =>
      logger.warn(label)
      r.foreach(a => logger.warn(a.toString))
      logger.warn("Generated query:")
      logger.warn(sqlQuery.statements.mkString("\n"))
      logger.warn("")
    }
  }

  private def runAndLogResults[R](label: String, query: DBIOAction[Seq[R], NoStream, _])(implicit logger: Logger): Future[Unit] = {
    db.run(query).map { r =>
      logger.warn(label)
      r.foreach(a => logger.warn(a.toString))
      logger.warn("")
...
object SlickTests extends App with Schema with Queries {

  val db = Database.forConfig("slick.db", ConfigFactory.load())

  val jdbcProfile = PostgresProfile

  implicit lazy val root: Logger = LoggerFactory.getLogger(s"${this.getClass.getCanonicalName}".replace("$", ""))
